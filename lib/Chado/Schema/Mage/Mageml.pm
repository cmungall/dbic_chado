package Chado::Schema::Mage::Mageml;

use strict;
use warnings;

use base 'DBIx::Class';

__PACKAGE__->load_components("Core");
__PACKAGE__->table("mageml");
__PACKAGE__->add_columns(
  "mageml_id",
  {
    data_type => "integer",
    default_value => "nextval('mageml_mageml_id_seq'::regclass)",
    is_nullable => 0,
    size => 4,
  },
  "mage_package",
  {
    data_type => "text",
    default_value => undef,
    is_nullable => 0,
    size => undef,
  },
  "mage_ml",
  {
    data_type => "text",
    default_value => undef,
    is_nullable => 0,
    size => undef,
  },
);
__PACKAGE__->set_primary_key("mageml_id");
__PACKAGE__->add_unique_constraint("mageml_pkey", ["mageml_id"]);
__PACKAGE__->has_many(
  "magedocumentations",
  "Chado::Schema::Mage::Magedocumentation",
  { "foreign.mageml_id" => "self.mageml_id" },
);


# Created by DBIx::Class::Schema::Loader v0.04005 @ 2009-06-20 19:31:25
# DO NOT MODIFY THIS OR ANYTHING ABOVE! md5sum:XIDmvLwItb0b1aSlbLmSQA


# You can replace this text with custom content, and it will be preserved on regeneration
1;
